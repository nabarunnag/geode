...
public class PortfolioPdx implements Serializable, PdxSerializable {
  ...
  public PortfolioPdx(int i) {
    ...
    type = "type" + (i % 3);
    position1 = new PositionPdx(secIds[PositionPdx.cnt % secIds.length], PositionPdx.cnt * 1000D);
    if (i % 2 != 0) {
      position2 = new PositionPdx(secIds[PositionPdx.cnt % secIds.length], PositionPdx.cnt * 1000D);
    } else {
      position2 = null;
    }

    positions.put(secIds[PositionPdx.cnt % secIds.length],
        new PositionPdx(secIds[PositionPdx.cnt % secIds.length], PositionPdx.cnt * 1000D));
    positions.put(secIds[PositionPdx.cnt % secIds.length],
        new PositionPdx(secIds[PositionPdx.cnt % secIds.length], PositionPdx.cnt * 1000D));
  ...
  public PortfolioPdx(int i, int j) {
    ...
    for (int k = 0; k < position3.length; k++) {
      PositionPdx p = new PositionPdx(secIds[k], (k + 1) * 1000D);
      p.portfolioId = (k + 1);
      this.position3[k] = p;
    }
  }
