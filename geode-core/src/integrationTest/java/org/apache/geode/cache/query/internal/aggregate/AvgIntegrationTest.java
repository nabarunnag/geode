...
public class AvgIntegrationTest extends AggregateFunctionQueryBaseIntegrationTest {
  ...
  private void prepareStructures() {
    ...
    queries.put("SELECT AVG(p.ID) FROM /" + firstRegionName + " p",
        downCast(supplierOne.get().mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    queries.put("SELECT AVG(p.ID) FROM /" + firstRegionName + " p WHERE p.ID > 0",
        downCast(supplierOne.get().filter(p -> p.getAcctBase() > 0).mapToInt(Portfolio::getAcctBase).average()
            .orElse(Double.NaN)));
    queries.put("SELECT AVG(p.ID) FROM /" + firstRegionName + " p WHERE p.ID > 50",
        downCast(supplierOne.get().filter(p -> p.getAcctBase() > 50).mapToInt(Portfolio::getAcctBase).average()
            .orElse(Double.NaN)));
    queries.put(
        ...
        downCast(supplierOne.get().filter(p -> p.getAcctBase() > 0 || p.isActive())
            .mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    queries.put(
    ...
    queries.put("SELECT AVG(p.ID) FROM /" + firstRegionName + " p WHERE NOT (p.ID > 5)",
        downCast(supplierOne.get().filter(p -> p.getAcctBase() <= 5).mapToInt(Portfolio::getAcctBase).average()
            .orElse(Double.NaN)));

    // Distinct Queries
    queries.put("SELECT AVG(DISTINCT p.ID) FROM /" + firstRegionName + " p WHERE p.ID > 0",
        downCast(
            supplierOne.get().filter(p -> p.getAcctBase() > 0).filter(distinctByKey(Portfolio::getAcctBase))
                .mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    ...
    queries.put("SELECT DISTINCT AVG(p.ID) FROM /" + firstRegionName
        + " p, p.positions.values pos WHERE p.ID > 0 AND pos.secId = 'IBM'",
        downCast(supplierOne.get().filter(p -> p.getAcctBase() > 0 && p.getPositions().containsKey("IBM"))
            .distinct().mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    queries.put("SELECT AVG(p.ID) FROM /" + firstRegionName
    ...
    equiJoinQueries.put("SELECT AVG(p.ID) from /" + firstRegionName + " p, /" + secondRegionName
        ...
        downCast(supplierOne.get().filter(p -> regionTwoLocalCopy.containsKey(p.getAcctBase()))
            .filter(p -> p.getAcctBase() > 0).mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    equiJoinQueries.put("SELECT AVG(p.ID) from /" + firstRegionName + " p, /" + secondRegionName
        ...
        downCast(supplierOne.get()
            ...
            .filter(p -> p.getAcctBase() > 20).mapToInt(Portfolio::getAcctBase).average().orElse(Double.NaN)));
    equiJoinQueries.put("SELECT AVG(p.ID) from /" + firstRegionName + " p, /" + secondRegionName
        + " e WHERE p.ID = e.ID AND p.ID > 0 AND p.status = 'active'",
        downCast(supplierOne.get().filter(p -> regionTwoLocalCopy.containsKey(p.getAcctBase()))
            .filter(p -> p.getAcctBase() > 0 && p.isActive()).mapToInt(Portfolio::getAcctBase).average()
            .orElse(Double.NaN)));
  }
