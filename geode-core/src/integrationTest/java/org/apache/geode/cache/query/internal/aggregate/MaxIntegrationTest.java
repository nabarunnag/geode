...
public class MaxIntegrationTest extends AggregateFunctionQueryBaseIntegrationTest {
  ...
  public void prepareStructures() {
    ...
    queries.put("SELECT MAX(ID) FROM /" + firstRegionName,
        supplierOne.get().mapToInt(Portfolio::getAcctBase).max().orElse(-1));
    queries.put("SELECT MAX(ID) FROM /" + firstRegionName + " WHERE ID > 0",
        supplierOne.get().filter(p -> p.getAcctBase() > 0).mapToInt(Portfolio::getAcctBase).max().orElse(-1));
    ...
    queries.put("SELECT MAX(p.getType()) FROM /" + firstRegionName
        ...
        supplierOne.get().filter(p -> p.getAcctBase() > 0 || p.isActive())
            .max(Comparator.comparing(Portfolio::getType)).map(Portfolio::getType).orElse(""));
    queries.put("SELECT MAX(p.getType()) FROM /" + firstRegionName
        + " p WHERE p.ID > 0 OR p.status LIKE 'ina%'",
        supplierOne.get().filter(p -> p.getAcctBase() > 0 || p.login.startsWith("ina"))
            .max(Comparator.comparing(Portfolio::getType)).map(Portfolio::getType).orElse(""));
    queries.put(
        "SELECT MAX(p.shortID) FROM /" + firstRegionName + " p WHERE p.ID IN SET(1, 2, 3, 4, 5)",
        supplierOne.get().filter(p -> Arrays.asList(1, 2, 3, 4, 5).contains(p.getAcctBase()))
    ...
    queries.put("SELECT MAX(p.shortID) FROM /" + firstRegionName + " p WHERE NOT (p.ID > 5)",
        supplierOne.get().filter(p -> p.getAcctBase() <= 5).max(Comparator.comparing(p -> p.shortID))
    ...
    queries.put("SELECT MAX(p.ID) FROM /" + firstRegionName
        ...
        supplierOne.get().filter(p -> p.getAcctBase() > 0 && p.getPositions().containsKey("IBM"))
            .mapToInt(Portfolio::getAcctBase).max().orElse(-1));
    queries.put("SELECT MAX(p.getType()) FROM /" + firstRegionName
    ...
    queries.put("SELECT MAX(p.status) FROM /" + firstRegionName
        + " p, p.positions.values pos WHERE p.ID > 0 AND pos.secId = 'IBM'",
        supplierOne.get().filter(p -> p.getAcctBase() > 0 && p.getPositions().containsKey("IBM"))
            .max(Comparator.comparing(p -> p.login)).map(p -> p.login).orElse(""));
    ...
    queries.put("SELECT MAX(p.shortID) FROM /" + firstRegionName
        ...
        supplierOne.get()
            .filter(p -> p.getAcctBase() > 0 || p.isActive() || !p.isActive()
    ...
    equiJoinQueries.put("SELECT MAX(p.shortID) from /" + firstRegionName + " p, /"
        ...
        supplierOne.get().filter(p -> regionTwoLocalCopy.containsKey(p.getAcctBase()))
            .filter(p -> p.getAcctBase() > 0).max(Comparator.comparing(p -> p.shortID))
            .map(p -> p.shortID).orElse((short) -1));
    equiJoinQueries.put("SELECT MAX(p.shortID) from /" + firstRegionName + " p, /"
        ...
        supplierOne.get()
            ...
            .filter(p -> p.getAcctBase() > 20).max(Comparator.comparing(p -> p.shortID))
            .map(p -> p.shortID).orElse((short) -1));
    equiJoinQueries.put("SELECT MAX(p.shortID) from /" + firstRegionName + " p, /"
        + secondRegionName + " e WHERE p.ID = e.ID AND p.ID > 0 AND p.status = 'active'",
        supplierOne.get().filter(p -> regionTwoLocalCopy.containsKey(p.getAcctBase()))
            .filter(p -> p.getAcctBase() > 0 && p.isActive()).max(Comparator.comparing(p -> p.shortID))
            .map(p -> p.shortID).orElse((short) -1));
  }
