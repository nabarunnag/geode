...
abstract class WANRollingUpgradeDUnitTest2 implements Serializable {
  ...
  static void createGatewayReceiver() {
    int port = AvailablePortHelper.getRandomAvailableTCPPort();
    GatewayReceiverFactory gwReceiver =
        ClusterStartupRule.getCache().createGatewayReceiverFactory();
    gwReceiver.setStartPort(port);
    gwReceiver.setEndPort(port);
    gwReceiver.create();
  }

  static String getCreateGatewaySenderCommand(String id, int remoteDsId) {
  ...
  static void createGatewaySender(String id, int remoteDistributedSystemId,
    ...
    GatewaySenderFactory gsf = ClusterStartupRule.getCache().createGatewaySenderFactory();
    gsf.setParallel(true);
    gsf.create(id, remoteDistributedSystemId);
  }

  void createPartitionedRegion(String regionName, String gatewaySenderId) {
  ...
  protected Integer getEventsReceived(String regionName) {
    ...
    EventCountCacheListener cl =
        (EventCountCacheListener) region.getAttributes()
            .getCacheListener();
  ...
  protected void createRegionAndConfigureGatwaysAndReceivers(String regionName, String site1SenderId,
                                                           int remoteID) {
    createGatewaySender(site1SenderId, remoteID,
        ParallelGatewaySenderQueue.DEFAULT_MESSAGE_SYNC_INTERVAL);
    createPartitionedRegion(regionName, site1SenderId);
    createGatewayReceiver();
  }

  protected void createClientRegionAndDoPuts(String regionName, int numPuts) {
    ClientCache clientCache = ClusterStartupRule.getClientCache();
    Region clientRegion =
        clientCache.createClientRegionFactory(ClientRegionShortcut.PROXY).create(regionName);
    for (int i = 0; i < numPuts; i++) {
      clientRegion.put(i, i);
    }
  }
  ...
  //
  // // Verify remote site did not receive any events. The events received were previously cleared
  // on
  ...
  // // from the
  // // underlying co-located region. Depending on the value of primaryOnly, it gets either the
  // local
